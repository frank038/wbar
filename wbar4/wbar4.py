...
# notification list: [folder name, NotificationItem]
LIST_NOTIFICATIONS = []
...
for el in _not_list:
    ...
    with open(os.path.join(_clip_dir,el,"notification"),"r") as _f:
        _not_text = _f.read()
    (n_app,n_summ,n_body) = _not_text.split("\n\n\n@\n\n\n")
    #
    _item = {"_app":n_app, "_summ":n_summ, "_body":n_body}
    LIST_NOTIFICATIONS.append([el, _item])
...
# class MyWindow(Gtk.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.not_icon_size2 = self.notification_conf["icon_size_list"]
            ...
            # notifications to not register
            self.not_skip_apps2 = []
            self.not_skip_apps2_tmp = []
        except:
            global USE_NOTIFICATIONS
            _error_log(WBNOTIFILEERR3)
            USE_NOTIFICATIONS = 0
        #
        # internal server
        if USE_NOTIFICATIONS == 1:
            ...
            # notifications to not to register
            _not_to_skip_path2 = os.path.join(_curr_dir,"configs","notifications_skipped2")
            _not_to_skip_ret2 = None
            if os.path.exists(_not_to_skip_path2):
                try:
                    with open(_not_to_skip_path2,"r") as _f:
                        _not_to_skip_ret2 = _f.read()
                except:
                    pass
            if _not_to_skip_ret2 != None:
                self.not_skip_apps2 = _not_to_skip_ret2.split("\n")
                if "" in self.not_skip_apps2:
                    self.not_skip_apps2.remove("")
        ...
        self._img_menu = Gtk.Image.new_from_paintable(_pb)
        self._img_menu.set_pixel_size(self.win_height-6)
        ...
        _pixbf = GdkPixbuf.Pixbuf.new_from_file_at_size(_icon_path, self.win_height,self.win_height)
        _pb = Gdk.Texture.new_for_pixbuf(_pixbf)
        self._img_other_menu = Gtk.Image.new_from_paintable(_pb)
        self._img_other_menu.set_pixel_size(self.win_height-6)
        self.otherbutton.connect('clicked', self.on_other_button)
        
        if self.menu_win_position == 0:
            self.menubutton.set_child(self._img_menu)
            self.otherbutton.set_child(self._img_other_menu)
        elif self.menu_win_position == 1:
            self.menubutton.set_child(self._img_other_menu)
            self.otherbutton.set_child(self._img_menu)
        ...
        if USE_NOTIFICATIONS == 1:
            ...
            self.not_alert_img.set_pixel_size(self.win_height-4)
            self.not_alert_img.set_visible(False)
            self.right_box.prepend(self.not_alert_img)
    ...
    def from_server_notifications(self, bus, message):
        ...
        if _app_name in self.not_skip_apps:
            return
        if _app_name in self.not_skip_apps2:
            return
    ...
    def on_toplevel_synced(self, context, toplevel):
        ...
        if toplevel.button._icon == None:
            ...
            if ret_icon:
                try:
                    if os.path.exists(ret_icon):
                        texture = Gdk.Texture.new_from_filename(ret_icon)
            ...
            if not ret_icon or not _img:
                try:
                    texture = Gdk.Texture.new_from_filename(os.path.join(_curr_dir,"icons","unknown2.svg"))
                    _img = Gtk.Image.new_from_paintable(texture)
                    _img.set_pixel_size(ICON_SIZE)
                    toplevel.button.set_child(_img)
                    toplevel.button._icon = 1
...
############# audio applet #########
    ...
    def set_volume(self, _sink, _descr):
        ...
        _level = int(round(max(_volume), 2)*100)
        _mute = _sink.mute
        self.volume_bar.set_value(_level/100)
        self.volume_bar.set_tooltip_text(WBVOLUME+str(_level)+"%"+"\n"+_descr)
        if _mute == 0:
            self.volume_bar.set_sensitive(True)
        elif _mute == 1:
...
#################### tray section #################
    ...
    def _activate_item(self, widget, id, popover):
        popover.popdown()
        try:
    ...
    def on_create_menu(self, menu, _data, popover=None):
        id = _data[0]
        _dict = _data[1]
        #
        if 'toggle-type' in _dict:
            ...
            if _toggle_type == 'checkmark':
                ...
                ckb.connect('toggled', self._activate_item, id, popover)
                menu.append(ckb)
            ...
            if _enabled:
                menu_item.connect('clicked', self._activate_item, id, popover)
            else:
            ...
            if 'children-display' in _dict:
                ...
                if _type == 'submenu':
                    ...
                    for el in _submenu_data:
                        self.on_create_menu(sub_menu, el, popover)
            else:
    ...
    def _create_menu(self, name,_menu, widget):
        ...
        for _data in _MENU:
            ...
            self.on_create_menu(main_box_popover, _data, popover)
        #
        popover.popup()
    ...
    # action on tray item e.g. open/close window applications
        ...
        if len(items) > len(old_items):
            ...
            if _found:
                ...
                if 'IconName' in _found[1]:
                    ...
                    try:
                        _pb = icon_theme.lookup_icon(_icon, None, self._app_icon_size, 1, Gtk.TextDirection.NONE, Gtk.IconLookupFlags.FORCE_REGULAR)
                    except:
                        _pb = None
                #
                                ...
                                if key == kk:
                                    if v != vv:
                                        if key == 'IconName':
                                            ...
                                            #
                                            if _icon == None or _icon == "" or _pb == None:
                                                if 'IconPixmap' in item:
                                                    if item['IconPixmap'] != '' or item['IconPixmap'] != []:
                                                        _icon_pix = item['IconPixmap'][0][2]
                                                        _icon_w = item['IconPixmap'][0][0]
                                                        _icon_h = item['IconPixmap'][0][1]
    ...
    def _item_changed22(self, sender, path):
        global items
        global old_items
        # 
        if len(items) > len(old_items):
...
################### end tray section #################
    ...
    def on_set_clock2(self, _pos):
        ...
        gesture = Gtk.GestureClick.new()
        gesture.set_button(3) # right
        gesture.connect("pressed", self.on_pressed_right_clock)
    ...
    def on_pressed_right_clock(self, o,n,x,y):
        ...
        _pop.set_has_arrow(False)
        _pop.set_halign(Gtk.Align.START)
        _pop.set_parent(self.clock_lbl)
        popover_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        _pop.set_child(popover_box)
        #
        button_reset = Gtk.Button(label=MWRESTARTCLOCK)
        button_reset.connect("clicked", self.on_reset_clock, _pop)
        popover_box.append(button_reset)
        _pop.popup()
        
    def on_reset_clock(self, _btn, _popover):
        _popover.popdown()
        if self._t_id:
    ...
    def set_not_window_size(self, _type, _value):
        if _type == "w":
            ...
            self.not_icon_size2_tmp = int(_value)
    ...
    def on_dialog_response(self, dialog, response_id):
        if response_id == Gtk.ResponseType.OK:
            ...
            if self.not_icon_size_tmp != 0:
                ...
                self.not_icon_size_tmp = 0
            if self.not_icon_size2_tmp != 0:
                self.notification_conf["icon_size_list"] = self.not_icon_size2_tmp
                self.not_icon_size2 = self.not_icon_size2_tmp
                self.not_icon_size2_tmp = 0
            ...
            if self.not_skip_apps_tmp != None:
                ...
                try:
                    ...
                    pass
                self.not_skip_apps_tmp = None
            if self.not_skip_apps2_tmp != None:
                _not_to_skip_ret2 = None
                try:
                    _not_to_skip_ret2 = self.not_skip_apps2_tmp.split(",")
                    if "" in _not_to_skip_ret2:
                        _not_to_skip_ret2.remove("")
                    _not_to_skip_path2 = os.path.join(_curr_dir,"configs","notifications_skipped2")
                    ff = open(_not_to_skip_path2, "w")
                    for el in _not_to_skip_ret2:
                        ff.write(el+"\n")
                    ff.close()
                    self.not_skip_apps2 = _not_to_skip_ret2
                except:
                    pass
                self.not_skip_apps2_tmp = None
            #
            self.save_conf()
        else:
...
class commandWin(Gtk.Window):
    def __init__(self, _parent, _command):
        ...
        #
        _cancel_btn = Gtk.Button(label=WBCANCEL)
    ...
    def on_c_btn(self, btn, _type):
        try:
            _f = None
            if _type == "lock":
                _ff = os.path.join(_curr_dir, "lockscreen.sh")
            elif _type == "logout":
...
class menuWin(Gtk.Window):
    def __init__(self, parent):
        ...
        _image = Gtk.Image.new_from_paintable(_pb)
        _image.set_pixel_size(int(self.BTN_ICON_SIZE/2))
        self.lockscreen.set_child(_image)
        self.lockscreen.set_tooltip_text(WBLOCKSCREEN)
        self.lockscreen.connect('clicked', self.on_service_btn, "lock")
        ...
        self.logout_btn.set_tooltip_text(WBLOGOUT)
        ...
        self.reboot_btn.set_tooltip_text(WBRESTART)
        ...
        self.shutdown_btn.set_tooltip_text(WBSHUTDOWN)
    ...
    def set_categories(self):
        ...
        _cat_tooltip = [WBCAT1,WBCAT2,WBCAT3,WBCAT4,WBCAT5,WBCAT6,WBCAT7,WBCAT8,WBCAT9,WBCAT10,WBCAT11,WBCAT12]
    ...
    def _find_the_icon(self,_icon):
        ...
        if _icon == None:
            try:
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(os.path.join(_curr_dir,"icons","Missed.svg"), self.ICON_SIZE, self.ICON_SIZE, True)
                _pb = Gdk.Texture.new_for_pixbuf(pixbuf)
                _i = Gtk.Image.new_from_paintable(_pb)
                return _i
            except:
        ...
        if _i == None:
            try:
                texture = icon_theme.lookup_icon(_icon, None, self._parent._app_icon_size, 1, Gtk.TextDirection.NONE, Gtk.IconLookupFlags.FORCE_REGULAR)
                _i = Gtk.Image.new_from_paintable(texture)
            except:
                _i = None
        return _i
...
class dialogMenu(Gtk.Window):
    def __init__(self, _parent, _type, _item):
        super().__init__()
        self._parent = _parent
        self._item = _item
        #
        ...
        btn_box = Gtk.Box.new(1,0)
        self.main_box.append(btn_box)
        
        if self._type == 1:
            btn_bookmark = Gtk.Button(label=WBTXT1)
            btn_box.append(btn_bookmark)
            btn_bookmark.connect("clicked", self.add_to_bookmark)
            
            if self._parent._parent.menu_editor:
                btn_menu = Gtk.Button(label=WBTXT2)
                btn_box.append(btn_menu)
                btn_menu.connect("clicked", self.modify_menu)
        
        elif self._type == 2:
            btn_bookmark = Gtk.Button(label=WBTXT3)
            btn_box.append(btn_bookmark)
            btn_bookmark.connect("clicked", self.remove_from_bookmark)
        
        btn_close = Gtk.Button(label=WBCLOSE)
        btn_box.append(btn_close)
        btn_close.connect("clicked", lambda w: self.close())
        
        self.set_visible(True)
    
    def add_to_bookmark(self, btn):
        try:
            with open(_menu_favorites, "a") as _f:
                _f.write(self._item)
                _f.write("\n")
            self._parent.bookmarks.append(self._item)
            # rebuild bookmarks
            # self._parent.populate_bookmarks_at_start()
            # self._parent.populate_category("Bookmarks")
        except Exception as E:
            self._parent.msg_simple(WBERROR+"\n"+str(E))
        self._parent._parent.MW = None
        self.close()
        
    def modify_menu(self, btn):
        if self._parent._parent.menu_editor == "__internal__":
            prog = os.path.join(_curr_dir, "appmenu", "appmenu6.py")
        else:
            prog = self._parent._parent.menu_editor
        try:
            Popen([prog, self._item])
        except Exception as E:
            self._parent.msg_simple(WBERROR+"\n"+str(E))
        self.close()
    
    def remove_from_bookmark(self, btn):
        try:
...
class clipboardWin(Gtk.Window):
    def __init__(self, parent):
        ...
        donotdisturb_btn = Gtk.ToggleButton(label=WBTXT103)
        ...
        empty_btn = Gtk.Button(label=WBTXT4)
...
class otherWin(Gtk.Window):
    def __init__(self, parent):
        ...
        self._stack.add_titled(_stack_vbox1,"Calendar",WBCALENDAR)
        
        _stack_vbox2 = Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=0)
        # _stack_vbox2.set_homogeneous(True)
        self._stack.add_titled(_stack_vbox2,"Notifications",WBNOTIFICATIONS)
        
        _stack_vbox3 = Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=0)
        self._stack.add_titled(_stack_vbox3,"Notes",WBNOTES)
        ...
        self.list_box.set_name("notlist")
        ...
        # for el in _not_list:
        # LIST_NOTIFICATIONS: [folder_name, {_app,_summ,_body}]
        for ell in LIST_NOTIFICATIONS:
            el = ell[0]
            row = Gtk.ListBoxRow()
            row.set_name("notrow")
            #
            ...
            # folder name of each notification stored
            row.iid = el
            
            _image_path = os.path.join(_clip_dir,el,"image.png")
            if os.path.exists(_image_path):
                _pix = GdkPixbuf.Pixbuf.new_from_file_at_scale(_image_path,self._parent.not_icon_size2,self._parent.not_icon_size2,True)
                ...
                if self._parent.not_rounded_img:
                    ...
                    #
                    _rs = Graphene.Size()
                    _rs.init(self._parent.not_icon_size2,self._parent.not_icon_size2)
                    _pb = _snapshot.to_paintable(_rs)
                #
                _img = Gtk.Image.new_from_paintable(_pb)
                _img.set_pixel_size(self._parent.not_icon_size2)
                hbox.append(_img)
        ...
        self.add_note = Gtk.Button(label=WBNOTENEW)
        self.add_note.connect('clicked', self.on_add_note)
        _stack_vbox3.append(self.add_note)
        
        self.show_hide_notes = Gtk.Button(label=WBNOTESHOWHIDE)
        ...
        separator = Gtk.Separator() # was disabled
        separator.set_orientation(Gtk.Orientation.HORIZONTAL)
        self.main_box.append(separator)
        ...
        conf_btn = Gtk.Button(label=WBCONFIGURATOR)
        conf_btn.connect('clicked', self.on_conf_btn)
        self.btn_box.append(conf_btn)
        
        self.dnd_btn = Gtk.Button(label=WBDONOTDISTURB)
        _dnd_file = os.path.join(_curr_dir,"do_not_disturb_mode")
        if os.path.exists(_dnd_file):
            self.dnd_btn.set_label(WBDONOTDISTURB1)
        self.dnd_btn.connect('clicked', self.on_dnd_btn)
        self.btn_box.append(self.dnd_btn)
        
        exit_btn = Gtk.Button(label=WBEXIT)
...
class DialogConfiguration(Gtk.Dialog):
    def __init__(self, parent):
        ...
        page1_label = Gtk.Label(label=WBPANEL)
        ...
        page2_label = Gtk.Label(label=WBMENU)
        ...
        page3_label = Gtk.Label(label=WBSERVICE)
        ...
        if self._parent.clipboard_use and USE_CLIPBOARD:
            ...
            page4_label = Gtk.Label(label=WBCLIPBOARD)
        ...
        page5_label = Gtk.Label(label=WBNOTIFICATIONS1)
        ...
        page6_label = Gtk.Label(label=WBOTHERSETTINGS)
        self.notebook.append_page(self.page6_box, page6_label)
        
        ##### PANEL
        # width - pixels to substract
        width_lbl = Gtk.Label(label=WBTXT102)
        width_lbl.set_tooltip_text(WBTXT5)
        ...
        # height
        size_lbl = Gtk.Label(label=WBTXT6)
        size_lbl.set_tooltip_text(WBTXT7)
        ...
        # position
        pos_lbl = Gtk.Label(label=WBTXT8)
        ...
        if USE_CLIPBOARD:
            ...
            clip_lbl.set_tooltip_text(WBTXT10)
        ...
        label1_lbl.set_tooltip_text(WBTXT12)
        ...
        # clock
        clock_lbl = Gtk.Label(label=WBTXT15)
        clock_lbl.set_tooltip_text(WBTXT16)
        ...
        ## MENU
        menu_lbl_w = Gtk.Label(label=WBTXT21)
        menu_lbl_w.set_tooltip_text(WBTXT22)
        ...
        menu_lbl_h = Gtk.Label(label=WBTXT23)
        menu_lbl_h.set_tooltip_text(WBTXT24)
        ...
        menu_lbl_ci = Gtk.Label(label=WBTXT25)
        menu_lbl_ci.set_tooltip_text(WBTXT26)
        ...
        menu_lbl_i = Gtk.Label(label=WBTXT27)
        menu_lbl_i.set_tooltip_text(WBTXT28)
        ...
        menu_lbl_wp = Gtk.Label(label=WBTXT31)
        ...
        menu_n_item_lbl.set_tooltip_text(WBTXT34)
        ...
        menu_editor_lbl = Gtk.Label(label=WBTXT35)
        menu_editor_lbl.set_tooltip_text(WBTXT36)
        ...
        ## SERVICE MENU
        service_lbl_w = Gtk.Label(label=WBTXT37)
        service_lbl_w.set_tooltip_text(WBTXT38)
        ...
        service_lbl_h = Gtk.Label(label=WBTXT6)
        service_lbl_h.set_tooltip_text(WBTXT39)
        ...
        lbl_note_show = Gtk.Label(label=WBTXT40)
        ...
        lbl_note_size.set_tooltip_text(WBTXT43)
        ...
        # tray menu size
        _tray_menu_width = Gtk.Label(label=WBTXT44)
        _tray_menu_width.set_tooltip_text(WBTXT45)
        ...
        _tray_menu_height = Gtk.Label(label=WBTXT46)
        _tray_menu_height.set_tooltip_text(WBTXT47)
        ...
        if USE_CLIPBOARD and self._parent.clipboard_use:
            clip_lbl_w = Gtk.Label(label=WBTXT37)
            clip_lbl_w.set_tooltip_text(WBTXT48)
            ...
            clip_lbl_h = Gtk.Label(label=WBTXT6)
            clip_lbl_h.set_tooltip_text(WBTXT49)
            ...
            if is_x11:
                ...
                clip_lbl_chars = Gtk.Label(label=WBTXT50)
                clip_lbl_chars.set_tooltip_text(WBTXT51)
            ...
            # max history
            clip_lbl_num = Gtk.Label(label=WBTXT52)
            clip_lbl_num.set_tooltip_text(WBTXT53)
            ...
            if is_x11:
                # chars preview
                clip_lbl_cp = Gtk.Label(label=WBTXT54)
                clip_lbl_cp.set_tooltip_text(WBTXT55)
        ...
        # window width
        not_lbl_w = Gtk.Label(label=WBTXT37)
        not_lbl_w.set_tooltip_text(WBTXT56)
        ...
        # window height
        not_lbl_h = Gtk.Label(label=WBTXT6)
        not_lbl_h.set_tooltip_text(WBTXT57)
        ...
        # icon size - notification window
        not_lbl_i = Gtk.Label(label=WBTXT58)
        not_lbl_i.set_tooltip_text(WBTXT59)
        ...
        # icon size - list
        not_i_spinbtn2 = Gtk.SpinButton.new_with_range(0,256,1)
        not_i_spinbtn2.set_value(self._parent.not_icon_size2)
        self.page5_box.attach_next_to(not_i_spinbtn2,not_i_spinbtn,1,1,1)
        not_i_spinbtn2.connect('value-changed', self.on_not_wh_spinbtn, "i2")
        not_i_spinbtn2.set_numeric(True)
        # # do not disturb
        ...
        # sounds
        not_lbl_sound = Gtk.Label(label=WBTXT62)
        ...
        not_lbl_pad.set_tooltip_text(WBTXT69)
        ...
        # bottom limit
        not_lbl_bottom = Gtk.Label(label=WBTXT70)
        not_lbl_bottom.set_tooltip_text(WBTXT71)
        ...
        # # volume change
        # vol_change_lbl = Gtk.Label(label=WBTXT72)
        # vol_change_lbl.set_tooltip_text(WBTXT73)
        # self.page5_box.attach(vol_change_lbl,0,10,1,1)
        # vol_change_lbl.set_halign(1)
        # vol_change_combo = Gtk.ComboBoxText.new()
        # vol_change_combo.append_text(WBNO)
        # vol_change_combo.append_text(WBYES)
        ...
        not_skip_lbl = Gtk.Label(label=WBTXT74)
        not_skip_lbl.set_halign(1)
        not_skip_lbl.set_tooltip_text(WBTXT75)
        ...
        # applications to not register only
        not_skip_lbl2 = Gtk.Label(label=WBTXT76)
        not_skip_lbl2.set_halign(1)
        not_skip_lbl2.set_tooltip_text(WBTXT77)
        self.page5_box.attach(not_skip_lbl2,0,12,1,1)
        not_skip_entry2 = Gtk.Entry.new()
        not_skip_entry2.set_text(",".join(self._parent.not_skip_apps2))
        not_skip_entry2.connect('changed', self.on_entry_not_skip2)
        self.page5_box.attach_next_to(not_skip_entry2,not_skip_lbl2,1,1,1)
        
        ## OTHER SETTINGS
        _lbl_advice = Gtk.Label(label=WBTXT78)
        _lbl_advice.set_halign(3)
        self.page6_box.attach(_lbl_advice,0,0,2,1)
        
        _lbl_pad = Gtk.Label(label=WBTXT79)
        ...
        _lbl_audio_start_lvl = Gtk.Label(label=WBTXT80)
        _lbl_audio_start_lvl.set_tooltip_text(WBTXT81)
        ...
        # tray enable/disable
        not_lbl_enabled = Gtk.Label(label=WBTXT84)
        not_lbl_enabled.set_tooltip_text(WBTXT85)
        ...
        # taskbar
        taskbar_lbl = Gtk.Label(label=WBTXT87)
        ...
        # launch mode
        launch_lbl = Gtk.Label(label=WBTXT90)
    ...
    def on_entry_not_skip2(self, _entry):
        self._parent.not_skip_apps2_tmp =_entry.get_text()
...
class Notifier(Service.Object):
    ...
    def create_not_win(self, _data, _urgency):
        ...
        if not _appname in self._parent.not_skip_apps2:
            if not _is_transient and STORE_NOTIFICATIONS:
                try:
                    ...
                    LIST_NOTIFICATIONS.insert(0, [_not_name, _not_item])
                    #
        ...
        if _summary:
            ...
            _lbl_summary.set_margin_start(4)
            _lbl_summary.set_margin_end(4)
            _lbl_summary.set_xalign(0)
            if _body == "":
                _lbl_summary.set_vexpand(True)
            else:
                _lbl_summary.set_margin_top(4)
        ...
        if _body:
            ...
            _lbl_body.set_xalign(0)
            _lbl_body.set_use_markup(True)
            _lbl_body.set_margin_start(4)
            _lbl_body.set_margin_end(4)
...
class Application(Gtk.Application):
    ...
    def __init__(self):
        super().__init__(application_id='org.example.wbar4',
